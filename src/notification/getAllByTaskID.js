...
module.exports = async ({ id, prismicConnection, userUID, page }) => {
  ...
  const notifications = await Notification.find({ taskID: id })
  ...
  //count all of them
  const notificationsSize = await Notification.count({ taskID: id });
